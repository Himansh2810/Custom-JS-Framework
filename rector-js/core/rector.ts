...
class RectorJS {
  ...
  #isRectorElement(node: RectorJS.Element) {
    if (
  ...
  #runApp(app: () => RectorJS.Element, lids: number | number[]) {
  ...
  #layoutExecution(layoutId: number, component: () => RectorJS.Element) {
  ...
  #layoutArrayExecution(layoutIds: number[], startCmp: () => RectorJS.Element) {
  ...
  #changeLayoutElement(layoutId: number, component: () => RectorJS.Element) {
  ...
  #showLoadingOverlay(Loader: () => RectorJS.Element) {
    this.#hideLoadingOverlay();
  ...
  public state<T>(
    ...
    if (!component || component.id == GLOBAL) {
      throw new RectorError(
        `[Rector.Error]: Can not use 'state()' out of component. Use 'createStore()' instead.`,
      );
    }
    ...
    if (this.#isListMarker(value)) {
      ...
      if (items !== null && !Array.isArray(items)) {
        throw new RectorError(
          `[Rector.Error]: In '${component.name}', at list(), List value must be an array or null.`,
        );
      }
  ...
  public createStore = <T extends Record<string, any>>(
    ...
    for (const stateName in config) {
      if (component.states[stateName]) {
        throw new RectorError(
          `[Rector.Error]: In GlobalStore, key '${stateName}' already exists in some store, try another key.`,
        );
      }
  ...
  public fromParent = (componentName?: string): ParentScope => {
    ...
    if (!componentName || typeof componentName !== "string") {
      throw new RectorError(
        `[Rector.Component]: At '${activeComponent.name}' fromParent(...),  Ancestor component name must be string.`,
      );
    }
  ...
  #getParentScope(
    ...
    Object.defineProperty(context, "state", {
        ...
        return new Proxy(targetComponent.stateObjects, {
          get(target, p: string) {
            ...
            if (!Object.hasOwn(target, p)) {
              throw new RectorError(
                `[Rector.Error]: State '${p}' doesn't exist on '${targetComponent.name}' component. declare one using state(initialVal,'${p}')`,
              );
            }
            return target[p];
          },
      ...
      set() {
        throw new RectorError(
          `[Rector.Error]: ParentScope can't set explicitly.`,
        );
      },
      enumerable: true,
    });
      ...
      get() {
        return new Proxy(targetComponent.listObjects, {
          get(target, p: string) {
            ...
            if (!Object.hasOwn(target, p)) {
              throw new RectorError(
                `[Rector.Error]: List '${p}' doesn't exist on '${targetComponent.name}' component. declare one using state(list(initialVal),'${p}')`,
              );
            }
            return target[p];
          },
  ...
  public suspense = (children: any, Loader: () => RectorJS.Element) => {
    ...
    if (Loader && typeof Loader === "function") {
      ...
      if (typeof node === "string" || typeof node === "number") {
        ...
        throw new RectorError(
          `[Rector.Error]: Loader in <Await> must return Html Element / Document fragment.`,
        );
      }
    }
  ...
  ): ((props: any) => RectorJS.AsyncComponent) => {
  ...
  async #resolveLazyComponent(children: any, container: DocumentFragment) {
    if (Array.isArray(children)) {
  ...
  #resolveChild(
    ...
    if (isLazyChildren(child)) {
      throw new RectorError(
        "[Rector.Component]: Async component (dynamic imported component) only allowed inside <Await>.",
      );
    }

    if (typeof child === "function" || isPlainObject(child)) {
      throw new RectorError(
        "[Rector.Component]: Functions and Objects are not allowed as children.",
      );
    }
...
export const state: typeof Rector.state = Rector.state.bind(Rector);
